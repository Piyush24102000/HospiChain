...
export default function Record_details() {
    ...
    const insu_details = async () => {
        try {
            ...
            await myContract.methods.insurance_details(id, appli, polinumber, insurer, politype, polilimit).send({ from: account });
            await myContract.methods.present_illness(id, complaints, duration).send({ from: account });
            await myContract.methods.past_illness(id, family, personal, drugs).send({ from: account });
            await myContract.methods.func_diagnosis(id, diag, pres).send({ from: account });
            await myContract.methods.treatment_summary(id, treat, datetreat, docid, hosid, disc, follow).send({ from: account });
        } catch (error) {
            console.log(error)
        }
    }
    return (
        <div>
            ...
            <Hospital_navbar />
            <h1><u>Patient Medical Record</u></h1> &nbsp;
            <form>

                <div className="form-group ml-3">
                    <h2><b>Previous dates of medical record updated</b></h2>
